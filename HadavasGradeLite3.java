// Hadavas Calculus I Grade Calculator Version 6.0 - Lite Edition Version 3.0


public class HadavasGradeLite3 {
	...
	// Method that tells the user the grade they need on the final exam to pass the class.
		public static void hadavasFinalGradeNeeded(double schemeOneGrade, double schemeTwoGrade) {
			...
			} while ((schemeOneGrade + (schemeOneFinalGradeNeededD * 0.25) < 60.0) && schemeOneFinalGradeNeededD <= 110.0);

			if (schemeOneFinalGradeNeededD > 110.0) {
			...
			if (schemeOneDPossible) {
				...
				} while ((schemeOneGrade + (schemeOneFinalGradeNeededC * 0.25) < 70.0) && schemeOneFinalGradeNeededC <= 110.0);

				if (schemeOneFinalGradeNeededC <= 110.0) {
				...
				if (schemeOneCPossible) {
					...
					} while ((schemeOneGrade + (schemeOneFinalGradeNeededB * 0.25) < 80.0) && schemeOneFinalGradeNeededB <= 110.0);

					if (schemeOneFinalGradeNeededB <= 110.0) {
					...
					if (schemeOneBPossible) {
						...
						} while ((schemeOneGrade + (schemeOneFinalGradeNeededA * 0.25) < 90.0) && schemeOneFinalGradeNeededA <= 110.0);

						if (schemeOneFinalGradeNeededA <= 110.0) {
			...
			} while ((schemeTwoGrade + (schemeTwoFinalGradeNeededD * 0.40) < 60.0) && schemeTwoFinalGradeNeededD <= 110.0);

			if (schemeTwoFinalGradeNeededD > 110.0) {
			...
			if (schemeTwoDPossible) {
				...
				} while ((schemeTwoGrade + (schemeTwoFinalGradeNeededC * 0.40) < 70.0) && schemeTwoFinalGradeNeededC <= 110.0);

				if (schemeTwoFinalGradeNeededC <= 110.0) {
				...
				if (schemeTwoCPossible) {
					...
					} while ((schemeTwoGrade + (schemeTwoFinalGradeNeededB * 0.40) < 80.0) && schemeTwoFinalGradeNeededB <= 110.0);

					if (schemeTwoFinalGradeNeededB <= 110.0) {
					...
					if (schemeTwoBPossible) {
						...
						} while ((schemeTwoGrade + (schemeTwoFinalGradeNeededA * 0.40) < 90.0) && schemeTwoFinalGradeNeededA <= 110.0);

						if (schemeTwoFinalGradeNeededA <= 110.0) {
			...
			if(!schemeOneDPossible) {
				schemeOneFinalGradeNeededD = 110.0;
			}
			if(!schemeOneCPossible) {
				schemeOneFinalGradeNeededC = 110.0;
			}
			if(!schemeOneBPossible) {
				schemeOneFinalGradeNeededB = 110.0;
			}
			if(!schemeOneAPossible) {
				schemeOneFinalGradeNeededA = 110.0;
			}
			if(!schemeTwoDPossible) {
				schemeTwoFinalGradeNeededD = 110.0;
			}
			if(!schemeTwoCPossible) {
				schemeTwoFinalGradeNeededC = 110.0;
			}
			if(!schemeTwoBPossible) {
				schemeTwoFinalGradeNeededB = 110.0;
			}
			if(!schemeTwoAPossible) {
				schemeTwoFinalGradeNeededA = 110.0;
			}
	...
	public static void main(String[] args) {
		...
		System.out.println("Juliana's Hadavas Calculus I Grade Calculator for Fall 2024 - Lite Edition! Version 6.0 \n");

		// Declare variables needed for the project.
		int attendancePoints, testDropped = 0;
		double examOne, examTwo, examThree, examFour, quiz, finalExam, schemeOne, schemeTwo, finalGrade;
		double examOnePercent, examTwoPercent, examThreePercent, examFourPercent, quizPercent, finalExamPercent;
		...
		if (argsLength == 1 && args[0].equals("help")) { // This section runs if the user only supplied one argument and it's "help".
			...
			System.out.println("To use the program, after the program name in the command line (HadavasGradeLite.java), input <Exam 1 Score> <Exam 2 Score> <Exam 3 Score> <Exam 4 Score> <Quiz Score> <Attendance Points> <Final Exam Score>. ");
		...
		} else if (argsLength == 6 || argsLength == 7) { // Runs if the user supplies 6 or 7 arguments.
			...
			if (canParseDouble(args[0]) && canParseDouble(args[1]) && canParseDouble(args[2]) && canParseDouble(args[3]) && canParseDouble(args[4]) && canParseInt(args[5]) && finalExamValid) {
				valid = true;
				...
				quiz = Double.parseDouble(args[4]);
				attendancePoints = Integer.parseInt(args[5]);

				// If any of the exam grades are lower than 0 and higher than 100.0 (110.0 for Exam 4) and or if the attendance value is higher than 12 or less than 0, then the value valid is set to false.
				if ((examOne < 0 || examOne > 110.0) || (examTwo < 0 || examTwo > 110.0) || (examThree < 0 || examThree > 11.0) || (examFour < 0 || examFour > 110.0) || (quiz < 0 || quiz > 110.0) || (attendancePoints < 0 || attendancePoints > 12)) {
					valid = false;
				}
				if (valid) { // Only runs if the values given from the user are valid.
					...
					schemeOne = examOnePercent + examTwoPercent + examThreePercent + examFourPercent + quizPercent + attendancePoints;
					...
					if (argsLength == 6) { // Runs only if the user supplied 6 arguments.

						// Calculate the current grade average based on test one, two, three, four, and your attendance points.
						double midtermPoints = examOnePercent + examTwoPercent + examThreePercent + examFourPercent + quizPercent + attendancePoints;
						double midtermGrade = (midtermPoints / 75) * 100;
					...
					} else { // Runs only if the user supplied 7 arguments.
						
						finalExam = Double.parseDouble(args[6]); // Pulls the final exam grade the user supplied in the arguments.
						
						if (finalExam < 0 || finalExam > 110.0) { // Changes the value valid to false if the final exam grade is less than 0 or higher than 100.0.
							valid = false;
						}
